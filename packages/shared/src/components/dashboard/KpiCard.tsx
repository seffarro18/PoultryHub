...
export default function KpiCard({ data }: { data: KpiCardData }) {
  ...
  const hasTrend = data.sparkline !== undefined && data.deltaPercent !== undefined;
  const deltaColor = data.isPositive ? "var(--color-success)" : "var(--color-danger)";
  const sparkData = data.sparkline?.map((value, i) => ({ i, value })) ?? [];
  const statusColor = data.status ? STATUS_COLOR[data.status] : "var(--color-muted)";
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] p-4">
      ...
      <div>
        ...
        <p className="mt-0.5 text-2xl font-semibold text-[var(--color-foreground)]">
      ...
      {hasTrend && (
        <div className="flex items-end justify-between gap-2">
          ...
          >
            {(data.deltaPercent ?? 0) >= 0 ? <ArrowUpRight size={13} /> : <ArrowDownRight size={13} />}
            {Math.abs(data.deltaPercent ?? 0)}%
          </span>
