...
/** Active/inactive farms only — an archived farm shouldn't be offered as a place to assign someone or record new activity. */
export async function listFarms(): Promise<Farm[]> {
  const { data, error } = await supabase.from("farms").select("id, name").neq("status", "archived").order("name");
