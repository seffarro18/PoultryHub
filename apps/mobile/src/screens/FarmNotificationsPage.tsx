...
import { AlertCircle, Archive, Bell, Check, Loader2, Send, ShieldAlert, Trash2 } from "lucide-react";
import {
  archiveNotification,
  deleteNotification,
  listNotifications,
...
import SendNotificationDrawer from "../components/notifications/SendNotificationDrawer";
import { FARM_ADMIN_CATEGORIES, NOTIFICATION_CATEGORY_META, type AppNotification, type NotificationCategory } from "@poultryhub/shared/types/notification";
...
export default function FarmNotificationsPage() {
  ...
  const handleMarkAsRead = async (id: string) => {
    ...
    try {
      ...
      console.error("[FarmNotificationsPage] mark as read failed:", err);
    } finally {
      setBusyId(null);
    }
  };
  ...
  const handleArchive = async (id: string) => {
    ...
    try {
      ...
      console.error("[FarmNotificationsPage] archive failed:", err);
    } finally {
      setBusyId(null);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      console.error("[FarmNotificationsPage] delete failed:", err);
    } finally {
      setBusyId(null);
    }
  };
  ...
  const handleMarkAllAsRead = async () => {
    try {
      ...
      console.error("[FarmNotificationsPage] mark all as read failed:", err);
    }
  };
      ...
      <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-card)] px-6 py-20 text-center">
        ...
        <div>
          ...
          <p className="mt-1.5 max-w-sm text-sm text-[var(--color-muted)]">
            Ask your Super Admin to assign your account to a farm before you can manage notifications.
          </p>
        </div>
      </div>
    );
    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-muted)]">Alerts for your farm, and reminders you send to staff.</p>
        </div>
